...
class TextNormalizer:
    ...
    _AMHARIC_PATTERN = re.compile(r"[\u1200-\u137f\u1380-\u139f\u2d80-\u2ddf\uab00-\uab2f]")
    _AMHARIC_PUNCT = re.compile(r"[።፣፤፥፦፧፨]")
    ...
    def __init__(self, preferred_language: str | None = None):
        ...
        self._am_cleanup_pattern = re.compile("|".join(re.escape(p) for p in self.am_char_rep_map.keys()))
    ...
    def is_amharic(self, text: str) -> bool:
        """Check if text contains Amharic script (Ge'ez/Ethiopic)"""
        if self._AMHARIC_PATTERN.search(text):
            return True
        if self._AMHARIC_PUNCT.search(text):
            return True
        return False
    ...
    def normalize_amharic(self, text: str) -> str:
        ...
        if not text:
            return ""
        # Remove speaker tags
        text = re.sub(r"^\s*(?:speaker|spk|ተናጋሪ)\s*\d+\s*[:：]\s*", "", text, flags=re.IGNORECASE)
        # Normalize Unicode (NFC for Amharic to ensure proper character composition)
        text = unicodedata.normalize("NFC", text)
        # Normalize whitespace
        text = re.sub(r"\s+", " ", text)
        # Replace Amharic punctuation
        text = self._am_cleanup_pattern.sub(lambda x: self.am_char_rep_map[x.group()], text)
        return text.strip()
    ...
    def normalize(self, text: str, language: str | None = None) -> str:
        ...
        if lang == "am":
            return self.normalize_amharic(text)
